...
const useNavigation = selectedCell => {
  ...
  const columns = useStore(store, (0, _react.useMemo)(() => state => state.columns, []));
  const hiddenRows = useStore(store, (0, _react.useMemo)(() => state => state.hiddenRows, []));
  ...
  const getNextOffset = (0, _react.useCallback)(_ref => {
    let {
      columnIndex,
      rowIndex
    } = _ref;
